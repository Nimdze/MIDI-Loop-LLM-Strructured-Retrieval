#!/usr/bin/env python3
"""Test 6 — Vague / non-musical / random queries.

50 non-standard, rare, vague or completely non-musical queries. There are NO
expected targets — these test graceful degradation: does the model stay
conservative (few/no tags, low confidence) rather than confidently over-tag
nonsense? Evaluated manually and by an LLM judge using
``evaluation_6_vague_queries_scoring.md``.

Semantic analysis enabled so each tag records WHY the model
chose it. Neutral family hint ("both") so the model self-classifies.

Runner writes: results.json, to_evaluate.json (judge input), distance_report.json.
"""
...
BANK = Path(__file__).resolve().parent / "evaluation_6_vague_queries.json"
SCORING_MD = Path(__file__).resolve().parent / "evaluation_6_vague_queries_scoring.md"
CHECKPOINT = Path(__file__).resolve().parent / ".evaluation_6_vague_queries_checkpoint.json"
PHASE = "evaluation_5"
EXP_DIR = "evaluation_vague_queries_6"
CONCURRENCY = 20
...
def _distance_report(bank: list[dict]) -> dict:
    ...
    return {"queries": rows, "mean_novelty": round(total / len(rows), 4) if rows else 0}
...
def main() -> int:
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix="midi_llm_evaluation6q_"))
