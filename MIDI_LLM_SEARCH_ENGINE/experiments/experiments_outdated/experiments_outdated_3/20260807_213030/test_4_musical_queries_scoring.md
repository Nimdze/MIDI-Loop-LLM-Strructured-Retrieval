...
`family_classification` (drums/pitched), `interpretation` (the model's own prose
reading of the query), `targets` (`concept` + `level`), and `semantic_analysis`
(the model's per-concept reasoning — the "why" for each tag).
...
[
  {
    "id": "mus_001",
    "family": 4, "relevance": 5, "coverage": 4, "levels": 5, "reasoning": 4, "over_tag": 5,
    "verdict": "hit",
