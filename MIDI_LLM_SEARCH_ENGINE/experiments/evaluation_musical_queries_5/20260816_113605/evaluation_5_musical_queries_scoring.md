# LLM-as-Judge Scoring Instructions — Test 4: Natural Musical Queries
...
received **only the query** — there are no expected targets. Your job is to
...
1. **Family classification** (`family`): is the model's drums/pitched choice
   sensible for this query? (Score 5 if sensible, 3 if debatable but defensible,
   1 if clearly wrong, N/A if the query spans both — then score 5.)
...
5. **Reasoning quality** (`reasoning`): do the `semantic_analysis` explanations
   convincingly justify each tag from the query text? Penalize hand-wavy or
   copy-pasted reasoning that doesn't reference the query.
6. **Over-tagging** (`over_tag`): did the model add tags that are *not* implied
   ...
   clear, 2 = several, 1 = egregious.
7. **Importance calibration** (`importance`): are the `importance` weights (1–5)
   sensible — do the most salient concepts get the highest importance, and
   peripheral/incidental ones lower? Penalize flat/uninformative weights (all
   equal, or a peripheral tag ranked above the query's core concept). 5 =
...
- `hit` — the interpretation is clearly right and musically useful (axes mostly 4–5).
- `partial` — mostly right but misses something or includes a questionable tag.
- `miss` — the interpretation is wrong or largely off-target.
- `over-tagged` — right idea but meaningful extra tags were invented.
